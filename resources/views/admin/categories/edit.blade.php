                ...
                <form method="POST" action="{{ route('admin.categories.update', ['categoryId' =>  $category->id]) }}" class="flex flex-wrap -m-2">
                    ...
                    <div class="p-2 w-full">
                        <div class="relative">
                            ...
                            <input type="text" id="name" name="name" value="{{ is_null(old('name')) ? $category->name : old('name')}}"
                    ...
                    <div class="p-2 w-full">
                        <div class="relative">
                            ...
                            <textarea id="description" name="description"
                                class="w-full bg-gray-50 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out">{{ is_null(old('description')) ? $category->description : old('description') }}</textarea>
                        </div>
                        {{-- バリデーションエラーメッセージ表示 --}}
                        @error('description')
                            <div class="text-red-300">{{ $message }}</div>
